...
CREATE TEMP TABLE arrtest2 (i integer ARRAY[4], f float8[], n numeric[], t text[], d timestamp[]) DISTRIBUTED FULLY;
...
-- some more test data
CREATE TEMP TABLE arrtest_f (f0 int, f1 text, f2 float8) DISTRIBUTED FULLY;
...
CREATE TEMP TABLE arrtest_i (f0 int, f1 text, f2 int) DISTRIBUTED FULLY;
...
CREATE TEMP TABLE arraggtest ( f1 INT[], f2 TEXT[][], f3 FLOAT[]) DISTRIBUTED FULLY;
...
create table comptable (c1 comptype, c2 comptype[], distkey int4) distributed fully;
...
create temp table t1 (f1 int8_tbl[], distkey int4) distributed fully;
